...
public:
    ...
    GvspImageGeometry(uint w=0, uint h=0, quint32 p=0):width(w),height(h),pixelFormat(p){}

    bool operator==(GvspImageGeometry const &other) const
    {return (other.width==this->width)&&(other.height==this->height)&&(other.pixelFormat==this->pixelFormat);}
    bool operator!=(GvspImageGeometry const &other) const
    {return (other.width!=this->width)||(other.height!=this->height)||(other.pixelFormat!=this->pixelFormat);}
};
...
class MemoryBlock {
public:
    MemoryBlock(quint8 *d = 0, size_t s = 0):p(d),size(s){}
    quint8 *p;
    size_t size;
};
